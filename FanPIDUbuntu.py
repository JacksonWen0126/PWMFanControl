...
fan.start(0)                   # Generate a PWM signal with a 0% duty cycle 
...
def get_temp():                             # Function to read in the CPU temperature and return it as a float in degrees celcius
    ...
    #cmd = "sensors | grep -i temp1 |awk '{print $2}' | cut -b 2-5"
    output=subprocess.run("sensors | grep -i temp1 |awk '{print $2}' | cut -b 2-5", shell=True,capture_output=True)
    temp_str = output.stdout.decode()
    try:
